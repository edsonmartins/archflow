...
- ✅ **Nice-to-haves fechados** — (1) cobertura de adapters era falso-positivo de métrica por arquivo (pgvector 55 testes, pinecone 36, redis 27, memory 17 — substancial); (2) tipo `Secret` + `ConfigSecrets.redactForLogging` no langchain4j-core (credenciais nunca em log; aplicado nos adapters OpenAI/Anthropic que guardam o config map em campo de vida longa); (3) `CanvasOutline` — visão textual acessível do grafo, navegável por teclado, alternativa ao React Flow mouse-only.
