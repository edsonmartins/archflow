...
- **Untrusted input boundaries**: three, all fenced with `UntrustedContentFence` (`br.com.archflow.api.trust`) — MCP tool results (`McpAgentRunner`); everything the browser sends to `AgUiAgentController` (`useAgentContext` values and `role:tool` messages); and **recovered context** handed to the agent loop (`Options.comContextoRecuperado` — in VendaX, the customer memory the Core sends in `VendaxInvoke.memoria`). The user's own turn is never fenced; they are the principal. In the agent loop the fence rule is always in the system message.
